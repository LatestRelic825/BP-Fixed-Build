...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(!ClientPrefs.classicScore) {
			...
			creditsWatermark = new FlxText(4, healthBarBG.y + 60, 0, credits, 16);
		...
		if(!ClientPrefs.classicScore) {
	    	songWatermark = new FlxText(5, textYPos, 0,
			SONG.song
			+ " - "
			...
			songWatermark = new FlxText(4, textYPos + 10, 0,
			SONG.song
			+ " - "
			+ (curSong.toLowerCase() != 'splitathon' ? (storyDifficulty == 3 ? "FINALE" : storyDifficulty == 2 ? "HARD" : storyDifficulty == 1 ? "NORMAL" : "EASY") : "FINALE")
			+ " | " + engineName + '0.6.2', 14);
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'devastation':
				composersWatermark = 'Hortas, Pyramix, Shredboi';
			case 'devastation-fanmade':
				composersWatermark = 'Ayop';
			case 'disposition' | 'disposition_but_awesome' | 'roundabout' | 'antagonism' |'antagonism-11-minutes' | 'upheaval' | 'antagonism-test':
  ...
  public function startVideo(name:String)
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), true, songLength / songMultiplier);
		#end
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer != null && startTimer.finished)
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), true, ((songLength - Conductor.songPosition) / songMultiplier >= 1 ? (songLength - Conductor.songPosition) / songMultiplier : 1) - ClientPrefs.noteOffset);
			}
			else
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
	...
	switch (SONG.song.toLowerCase())
	{
		...
		case 'devastation':
			switch (curBeat)
			{
				...
				case 744:
					...
					FlxTween.tween(blackScreen, {alpha:1}, 25.6);
				...
				case 824:
					...
					FlxTween.tween(blackScreen, {alpha:0}, 11.2);
				case 1336:
					FlxTween.tween(camHUD, {alpha:0}, 13.65);
					FlxTween.tween(blackScreen, {alpha:1}, 13.65);
			}
		case 'devastation-fanmade':
			switch (curBeat)
			{
		...
		case 'antagonism':
			switch(curBeat)
			{
				...
				case 4244:
					add(blackScreen);
					blackScreen.alpha = 0;
					FlxTween.tween(blackScreen, {alpha:1}, 5.48);
		...
		case 'upheaval':
			switch(curStep)
	    	{
				...
				case 6784:
					...
					blackScreen.alpha = 0;
				case 6785:
					FlxTween.tween(blackScreen, {alpha:1}, 21.2);
			}
		case 'lacuna':
			switch(curStep)
	    	{
		...
		if (ClientPrefs.goldenappleiconbounce) {
		...
		else if (!ClientPrefs.goldenappleiconbounce) {
		...
		else
		{
			...
			if (!paused)
			{
				...
				if(updateTime) {
					...
					secondsTotal = Std.int(secondsTotal / songMultiplier);
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					if(daNote.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
						...
						(SONG.song.toLowerCase() == 'devastation' && !SONG.notes[Math.floor(curStep / 16)].altAnim && !wtfThing) ? { // hi
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			switch (curSong.toLowerCase())
				{
					...
					// if ()
					if(!practiceMode || !cpuControlled || songMultiplier >= 1) {
						if (isStoryMode){
			...
			else
			{
				...
				{
				case 'demise pt 1':
					PlayState.SONG = Song.loadFromJson("demise-pt-2-null", "demise-pt-2");
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.visible = !ClientPrefs.hideRatings;
		rating.active = !ClientPrefs.hideRatings;
		if(ClientPrefs.ratinginHud) {
		...
		var comboSpr:FlxSprite = new FlxSprite();
			...
			comboSpr.visible = !ClientPrefs.hideRatings;
			comboSpr.active = !ClientPrefs.hideRatings;
			...
			if(ClientPrefs.ratinginHud) {
				...
				currentTimingShown.y -= ClientPrefs.comboOffset[1] + 10;
			} else {
				currentTimingShown.x = coolText.x + 30;
				currentTimingShown.y -= 0;
			}
		...
		for (i in seperatedScore)
		{
			...
			numScore.visible = !ClientPrefs.hideRatings;
			numScore.active = !ClientPrefs.hideRatings;
	...
	override function beatHit()
	{
		...
		if(curBeat % 2 == 0)
		{
			...
			if (bamburg != null)
			{
				if (bamburg.animation.curAnim.name != null && !bamburg.animation.curAnim.name.startsWith("sing") && !bamburg.stunned)
				{
					bamburg.dance();
				}
			}
	...
	override function sectionHit()
	{
		...
		if (SONG.notes[curSection] != null)
		{
			...
			{
				Conductor.changeBPM(SONG.notes[curSection].bpm, songMultiplier);
